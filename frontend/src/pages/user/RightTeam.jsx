...
const RightTeam = () => {
  ...
  useEffect(() => {
    const fetchRightTeam = async () => {
      if (!user?.userId) return;

      try {
        setLoading(true);
        // 🟢 FIX: Yahan type 'right' bhej rahe hain
        ...
        console.error("Right Downline loading error:", err); 
      } finally { 
        setLoading(false); 
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 font-sans min-h-screen">
      ...
      {/* 🟣 PURPLE THEME HEADER FOR RIGHT TEAM */}
      <div className="bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 p-6 sm:p-8 rounded-2xl text-white shadow-xl relative overflow-hidden mb-8 border-b-4 border-purple-400/30">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
                <h2 className="text-2xl sm:text-3xl font-bold uppercase tracking-tight">Right Downline</h2>
                <p className="text-purple-200 text-xs sm:text-sm font-medium uppercase mt-1">Your Right Team Network</p>
            </div>
          </div>
          <div className="bg-black/20 backdrop-blur-md px-6 py-3 rounded-xl border border-white/10 w-full md:w-auto text-center md:text-left">
             <span className="text-xs font-semibold uppercase text-purple-300 block mb-1">Right Business</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mt-8 relative z-10">
          <div className="bg-white/10 p-4 sm:p-5 rounded-xl border border-white/10 flex items-center gap-4">
            <div className="bg-purple-500/30 p-2.5 rounded-lg"><Users className="text-purple-300" size={24}/></div>
            <div>
                <p className="text-xs font-medium uppercase text-purple-200">Right Team Count</p>
          ...
          <div className="bg-white/10 p-4 sm:p-5 rounded-xl border border-white/10 flex items-center gap-4">
            ...
            <div>
                <p className="text-xs font-medium uppercase text-purple-200">Active IDs</p>
                <h3 className="text-xl font-bold text-emerald-400">{activeCount} Paid</h3>
            </div>
          </div>
          <div className="bg-white/10 p-4 sm:p-5 rounded-xl border border-white/10 flex items-center gap-4">
            <div className="bg-pink-500/30 p-2.5 rounded-lg"><TrendingUp className="text-pink-300" size={24}/></div>
            <div>
                <p className="text-xs font-medium uppercase text-purple-200">Today's Joins</p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 sm:py-2.5 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none text-sm sm:text-base text-gray-700 transition-colors"
          />
        </div>
        <select
          value={entriesPerPage}
          onChange={(e) => { setEntriesPerPage(Number(e.target.value)); setCurrentPage(1); }}
          className="w-full sm:w-auto px-4 py-2 sm:py-2.5 bg-white border border-gray-300 rounded-lg shadow-sm text-sm font-medium outline-none cursor-pointer hover:bg-gray-50 transition-colors focus:ring-2 focus:ring-purple-500"
        >
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        {loading ? (
          <div className="py-16 text-center flex flex-col items-center gap-3">
             ...
             <p className="font-medium text-gray-500 text-sm">Mapping Right Downline...</p>
          </div>
        ) : (
                ...
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((member, index) => (
                    <tr key={member._id || index} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-purple-600">{member.userId || "-"}</td>
