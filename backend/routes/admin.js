...
// GET: Admin ki All Package History
// 📦 GET /api/admin/all-package-history (For Total Top-Up Page)

// 📦 GET /api/admin/all-package-history
router.get('/all-package-history', verifyAdmin, async (req, res) => {
    try {
        const activations = await Transaction.find({ 
            type: 'PACKAGE_ACTIVATION',
            status: 'completed'
        }).sort({ createdAt: -1 }).lean();

        const userIds = [...new Set(activations.map(tx => tx.userId))];
        ...
        const result = activations.map(tx => ({
            _id: tx._id,
            userId: tx.userId,
        ...
        console.error('Error in /all-package-history:', err);
...
router.post("/credit", async (req, res) => {
  try {
    ...
    // 1️⃣ Strict Number Check for User ID
    ...
    // 3️⃣ Security Check (As per our established system)
    ...
    // 4️⃣ Validation & Total Calculation
    let totalToCredit = 0;
    const creditItems = Array.isArray(items) ? items : [];

    if (creditItems.length === 0) {
      return res.status(400).json({ success: false, message: "No income sources selected" });
    }

    for (const { source, amount } of creditItems) {
      const creditAmount = Number(amount);

      // Check valid sources according to our Schema (wallets.taskIncome, etc.)
      const validSources = ["taskIncome", "directIncome", "matchingIncome", "rankReward", "royaltyIncome"];
      
      if (creditAmount <= 0 || !validSources.includes(source)) {
        return res.status(400).json({
          success: false,
          message: `Invalid amount or source: ${source}`,
        });
      }

      // Balance check from user.wallets[source]
      if (creditAmount > (user.wallets[source] || 0)) {
        return res.status(400).json({
          success: false,
          message: `Insufficient balance in ${source}`,
        });
      }
      totalToCredit += creditAmount;
    }

    // Minimum limit check
    if (totalToCredit < 1) {
       return res.status(400).json({ success: false, message: "Minimum conversion is $1" });
    }

    // 5️⃣ Apply Credits & Deduct Income
    for (const { source, amount } of creditItems) {
      const creditAmount = Number(amount);
      
      // Deduct from sub-wallet
      user.wallets[source] -= creditAmount;
    }

    // Add total to Main Wallet (Top-up balance)
    user.walletBalance = (user.walletBalance || 0) + totalToCredit;

    // 6️⃣ Record Transaction (For Admin & User History)
    await Transaction.create({
      userId: user.userId,
      name: user.name, // Important for Admin Panel
      amount: totalToCredit,
      type: "INCOME_REINVEST", // 🟢 This matches your new Admin Component
      transactionType: "credit",
      walletType: "main_wallet",
      description: `Converted $${totalToCredit.toFixed(2)} from income to main wallet`,
      status: "completed"
    });
    ...
    res.json({
      success: true,
      message: "Income converted to main wallet successfully!",
      walletBalance: user.walletBalance,
      wallets: user.wallets // Updated sub-wallets
    });
    ...
    res.status(500).json({
      success: false,
      message: "Server error while processing conversion",
    });
  }
});
